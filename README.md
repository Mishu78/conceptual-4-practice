...
querySelector can select element using selectors like id,class,tag name.It returns the first matched element. querySelectorAll selects all matching elements and returns a Nodelist.
...
ans:It means adding one event listner to a parent element instead of adding many event listners to child elements. This is useful because it makes the code shorter and more efficient.
The elements which are created dynamically in that case it performs better.The parent can handle events for new elements without adding new event listner.So it improves the performance.
...
It actually stops the default action or behavior of an element.For example it can stop a link to open a new page.This is the main difference between them.
